...
class PlayState extends MusicBeatState
{
	...
	private var ratingGroup:FlxTypedGroup<FlxSprite>;

	private var enemyStrumTimers:Array<FlxTimer> = [];
	private var enemyStrumArray:Array<Bool> = [false, false, false, false];
	...
	private var healthBarPercent:Float = 0;
	...
	override public function update(elapsed:Float)
	{
		...
		var mult:Float = FlxMath.lerp(1, iconP1.scale.x, CoolUtil.boundTo(1 - (elapsed * 8), 0, 1));
		iconP1.scale.set(mult, mult);
		
		var mult:Float = FlxMath.lerp(1, iconP2.scale.x, CoolUtil.boundTo(1 - (elapsed * 8), 0, 1));
		iconP2.scale.set(mult, mult);

		var iconOffset:Int = 26;

		scoreLerp = Std.int(FlxMath.lerp(songScore, scoreLerp, CoolUtil.boundTo(1 - (elapsed * 15), 0, 1)));
				...
				if (!daNote.mustPress && daNote.wasGoodHit)
				{
					...
					enemyStrums.forEach(function(spr:FlxSprite)
					{
						if (Math.abs(daNote.noteData) == spr.ID)
						{
							...
							enemyStrumArray[spr.ID] = true;

							if(enemyStrumTimers[spr.ID] != null && enemyStrumTimers[spr.ID].active) enemyStrumTimers[spr.ID].cancel();
							
							enemyStrumTimers[spr.ID] = new FlxTimer().start(5 / 60, function(tmr:FlxTimer)
							{
								enemyStrumArray[spr.ID] = false;
							});
						}
					});
	...
	function endSong():Void
	{
		...
		if (isStoryMode)
		{
			...
			else
			{
				...
				if (SONG.song.toLowerCase() == 'eggnog' || SONG.song.toLowerCase() == 'south')
				{
				...
				{
					if(SONG.song.toLowerCase() == 'senpai' || SONG.song.toLowerCase() == 'roses' || SONG.song.toLowerCase() == 'south' || SONG.song.toLowerCase() == 'guns'){
	...
	{
		ratingGroup.forEachAlive(function(spr:FlxSprite):Void{
			spr.visible = false;
		});
	...
	function displayCombo():Void
	{
		...
		for (i in seperatedScore)
		{
			...
			if(misses <= 0){
				numScore.color = 0xFFE100;
				comboSpr.color = 0xFFE100;
			}
	...
	private function keyShit():Void
	{
		...
		if (pressArray.contains(true) && /*!boyfriend.stunned && */ generatedMusic)
		{
			...
			{
				for (shit in 0...pressArray.length)
					if (pressArray[shit] && !PreferencesMenu.getPref('ghost-tap')){
						trace('NOTTA GHOST TAPPERRR');
						noteMiss(shit);
					}
			}
		}
		...
		enemyStrums.forEach(function(spr:FlxSprite)
		{
			if(!enemyStrumArray[spr.ID]) spr.animation.play('static');
